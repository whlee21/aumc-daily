drop table if exists itfcdmpv532_daily.mt_rule_drug_exposure;

create table itfcdmpv532_daily.mt_rule_drug_exposure as
select d.uid, d.patient_id, d.order_day, d.order_cd, d.target_concept_id_1, d.source_concept_id, d.source_domain_id, d.target_domain_id, d.target_concept_seq
     , d.drug_start_dt, d.drug_end_dt, d.provider, d.reference_gb, d.method_cd, d.self_drug_yn
     ...
     , case when (lower(d.target_domain_id) = 'drug' or d.target_domain_id IS NULL) and d.quantity > 0 then d.quantity  --약인 경우에 0초과인 경우 그대로 가져오고
            when lower(d.target_domain_id) <> 'drug' and d.quantity >= 1 then d.quantity else 1 end quantity -- 약이 아닌 경우 1 이하이거나 null라면  1로 대체, 약이어도 음수이거나 null인 경우 1로 대체
            --(target null 포함 by lsh 20200618)
     , case when d.drug_start_dt is null                           then 'H' --시작일자가 null인 경우
            when d.drug_start_dt::date > d.drug_end_dt::date       then 'I' --시작일자가 종료일자보다 나중인 경우
            when d.drug_start_dt::date < d.birth_dt::date          then 'J' --시작일자가 생년월일보다 먼저인 경우
            when d.patient_id is null                              then 'K' --환자번호가 없는 경우
            when d.order_cd     is null                            then 'L' --처방코드가 없는 경우
            when extract(year from d.drug_start_dt)::integer - extract(year from d.birth_dt)::integer > 150 then 'M' --시작일자가 생년월일+150년보다 더 나중인 경우
  ...
  from (select t2.uid, t2.patient_id, t2.order_day, t2.order_cd, t2.target_concept_id_1, t2.source_concept_id, t2.source_domain_id, t2.target_domain_id, t2.target_concept_seq
             , t2.drug_start_dt, t2.provider, t2.quantity, t2.reference_gb, t2.method_cd, t2.self_drug_yn, t2.birth_dt, t2.filtering
             ...
             , t2.lot_number, t2.order_dr, t2.bill_order_gb, t2.verbatim_end_date   
             , case when coalesce(d2.death_datetime::date, to_date('9999-12-31', 'yyyy-mm-dd')) < t2.drug_end_dt                 then d2.death_datetime --사망일자가 종료일자보다 먼저일 경우 종료일자를 사망일자로 대체함
                    when (t2.drug_end_dt is null or t2.drug_end_dt::date = '9999-12-31'::date) and d2.death_datetime is not null then d2.death_datetime --종료일자가 불분명한 경우에 사망일자가 명확한 케이스에 한 하여 사망일자로 대체함
                    when t2.drug_end_dt is not null and t2.drug_end_dt > now()                                                   then now() 
                    else t2.drug_end_dt end as drug_end_dt
             , t2.ward_cd
          from (select case when t1.visit_gb_div in ('9201', '9203', '32037') and discharge_drug_yn <> 'Y' 
                            then t1.drug_start_dt --1 day 처방
                            else t1.drug_start_dt + (interval '1 day' * (t1.order_day - 1)) end as drug_end_dt --기존  t1.drug_start_dt를 date로 형변환하면 시간 데이터가 loss되어 로직 변경함
                     , case when t1.tot_order_qty is not null and t1.tot_order_yn = 'Y' -- 이승형 수정 "and t1.tot_order_yn = 'Y'" 
                            then t1.tot_order_qty --처방량 마스터 개념   
                  ...
                  from (
                        select d1.uid 
                             ...
                             , d1.order_qty1
                             , d1.order_qty2
                             , coalesce(d1.order_day,1) order_day
                             , d1.tot_order_yn
                             , p2.uid as person_id
                             , p2.birth_dt
                             ...
                             , case when c1.target_value = '1113'  --수술
                                    then coalesce(d1.operation_dt, d1.start_dt, d1.act_dt, d1.order_dt, d1.order_dt)
                                    when c1.target_value = '1114'  --마취
                                    then coalesce(d1.anesthesia_dt, d1.start_dt, d1.act_dt, d1.order_dt, d1.order_dt)
                                    else coalesce(d1.start_dt, d1.act_dt, d1.order_dt) end as drug_start_dt   
                             , case when c1.target_value = '1113'  --수술
                             ...
                             , case when d1.dc_yn1 = 'Y'        or d1.dc_yn2 = 'Y' /*or d1.dc_order_seq is not null 삭제 BY LSH 20200617*/ then 'A'  --취소관련
                                    when d1.cancel_yn = 'Y'     or d1.cancel_dt is not null                       then 'B'  --취소관련2
                                    when d1.prn_order_yn = 'Y' and d1.prn_act_yn = 'N'                            then 'C'  --PRN관련
                                    when d1.pre_order_yn = 'Y' and d1.pre_order_act_yn = 'N'                      then 'D'  --PRE관련
                                    when d1.act_yn is not null and d1.act_yn = 'N'                                then 'E'  --실시관련
                                    when c3.target_value = '9999'                                                 then 'F'  --정규처방이 아닌 경우
                                    when d1.order_cnt  < 0 or d1.order_qty1 < 0 or d1.order_qty2 < 0 
                                         or d1.order_day < 0 or d1.tot_order_qty < 0                              then 'G'  --음수값인 경우
                                    else 'N' end as filtering
                             , d1.ward_cd
                          from itfcdmpv532_daily.itf_drug_exposure d1 
                          ...
                          left join mapcdmpv532_daily.local_hash lh
                            on nullif(trim(d1.order_cd),'') = lh.local_cd1 and lh.source_domain_id = 'Drug'
