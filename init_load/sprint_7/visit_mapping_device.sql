drop table if exists itfcdmpv532_daily.visit_mapping_device;;

create table itfcdmpv532_daily.visit_mapping_device as
select tt.*
  from(
       select row_number() over(partition by v.device_exposure_id order by v.visit_gb_mapping,v.visit_occurrence_id, v.visit_detail_id desc) rank,*
         from(
              select d1.device_exposure_id
