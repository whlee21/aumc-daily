...
CREATE INDEX itf_slmicsnt_workarea_idx ON ods_daily.itf_slmicsnt USING btree (workarea, workdate, labno);
*****************************************************/

DROP TABLE IF EXISTS itfcdmpv532_daily.ITF_MEASUREMENT_7;;

CREATE TABLE itfcdmpv532_daily.ITF_MEASUREMENT_7
AS
select a.PATNO                                          as patient_id
      ...
      ,'Y'                                              as antibiotic_yn
      ,b.ANTICODE                                       as antibiotic_cd
      ,case when 
                 (
                  select count(*)
                    from ods_daily.SLMICSNT m
                   where m.WORKAREA = a.WORKAREA
                     and m.WORKDATE = a.WORKDATE
                     and m.LABNO    = a.LABNO
                     and m.SENSRSLT is not null
                     and m.SENSRSLT = m.SENSRSLTCD      
                 ) > 0 and      
                 (
                  select count(*)
                    from ods_daily.SLMICSNT m
                 ...
                 (
                  select count(*)
                   ...
                   where m.WORKAREA = a.WORKAREA
                     ...
                     and m.SENSRSLT is not null
                     and m.SENSRSLT = m.SENSRSLTCD
                     and regexp_match(m.SENSRSLT, '[^0-9]') IS NOT null
                 )
            then 'DISK'
            else 'MIC'
       end                                             as examination_gb
      ...
      ,'N'                                              as prn_order_yn
      ,'N'                                              as prn_act_yn
      ,case when b.DELID is null then case when c.CANCELID is null then 'N'
                                          else  'Y' end
...
select (SELECT last_value FROM etl_task_check_grp_id), 'itf_measurement_7' , 'itf_measurement_7', count(*) as cnt
from itfcdmpv532_daily.itf_measurement_7 ;