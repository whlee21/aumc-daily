...
DROP TABLE if exists itfcdmpv532_daily.itf_order;;

CREATE TABLE itfcdmpv532_daily.itf_order as
...
from ( 
    ...
    from (
        ...
        FROM (
                 ...
                 UNION ALL
                 SELECT *
                 FROM itfcdmpv532_daily.itf_order_6 A
             ) A
...
where 1=1
and COALESCE(bill_order_gb,'0') not in ('1','3','8')
;;

-----------------------------check cnt
insert into ods_daily.etl_task_check(task_grp_id, task_id, table_name, cnt)
select (SELECT last_value FROM etl_task_check_grp_id), 'itf_order' , 'itf_order', count(*) as cnt
from itfcdmpv532_daily.itf_order ;
